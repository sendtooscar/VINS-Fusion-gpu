...
std::mutex m_buf;
bool rtk_unreliable = true;  //checks fix status before using gps TODO  : send to config file // to cehck status of the GPS
bool use_gps = false;  // uses the raw rtk gps from fix
bool viz_gps = true;  // visualizes gps on rviz
bool viz_fast_gps = false; // update gps visualization at fast lio iniput

bool use_ppk = static_cast<bool>(USE_PPK);  //TODO  : send to config filet_I  // uses the ppk messge from folder
bool use_frl = static_cast<bool>(USE_FRL);  //TODO  : send to config file   // uses frl file from folder 
bool use_frl_rot = static_cast<bool>(USE_FRL_ROT); // this one makes it use only the rotation
// note GPS is updated only 10 times to initialize
bool use_mag_head = static_cast<bool>(USE_MAG);// this one performs a heading only update  (currently this one performs a heading and vio atti update)
bool use_vio_atti = false;// TODO: this one should performs a ref vector update (good for roll pitch global update using vio)
bool viz_ppk = static_cast<bool>(VIZ_PPK);  //TODO  : send to config file  // this visulizes the ppk path as ground truth
bool viz_frl = static_cast<bool>(VIZ_FRL);  //TODO  : send to config file  // this visulizes the frl path as ground truth
...
int map_theta = MAP_THETA;
bool map_theta_optimize = static_cast<bool>(MAP_THETA_OPTIMIZE);  //TODO  : send to config file  // this visulizes the ppk path as ground truth
std::string ppk_pos_file = PPK_PATH; //TODO  : send to config file
std::string frl_pos_file = FRL_PATH; //TODO  : send to config file
...
bool map_surround_only =false; // this makes the node only publish sections of the map every x scans
...
//std::string ppk_pos_file = "/storage_ssd/bell412_dataset1/bell412_dataset1_ppk.pos"; //TODO  : send to config file
...
pcl::PointCloud<PointType>::Ptr laserCloudFullMapTK(new pcl::PointCloud<PointType>());
...
void laserCloudFullResHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloudFullRes2)
{
    ...
    //3.iterate over global path 
     ...
     if (sync_found){
        ...
        //Eigen::Matrix4d Tmat_WL = Tmat_WB * Tmat_IL; // debugging
        ...
        if (!map_init){ 
            *laserCloudFullMapTK=*laserCloudFullRes;
    ...
    //if (frameCount % 50 == 0){
    //    pcl::io::savePCDFileBinary("/storage_ssd/nira-maps/HR-Map-"+std::to_string(frameCount)+".pcd", *laserCloudFullMapTK);
    //} 
    ...
    //8. publish map - limited to last 1000 scans  - put this in seperate thread if we are planning to save
     if (frameCount % 50 == 0 && keyframePoses.size()>0){       
       TicToc t_publish_cloud; 
        ...
        if(frameCount == 2600){   // dataset 1 : 520 ;dataset 6: 800; lighthouse:2600 
            laserCloudFullMap->clear();
...
int main(int argc, char **argv)
{
    ...
    //Eigen::Vector3d t_IL(1.0,2.0,3.0);
    ...
    use_ppk = static_cast<bool>(USE_PPK);  //TODO  : send to config filet_I  // uses the ppk messge from folder
    use_frl = static_cast<bool>(USE_FRL);  //TODO  : send to config file   // uses frl file from folder 
    use_frl_rot = static_cast<bool>(USE_FRL_ROT); // this one makes it use only the rotation
    use_mag_head = static_cast<bool>(USE_MAG);// this one performs a heading only update  (currently this one performs a heading and vio atti update)
    viz_ppk = static_cast<bool>(VIZ_PPK);  //TODO  : send to config file  // this visulizes the ppk path as ground truth
    viz_frl = static_cast<bool>(VIZ_FRL);  //TODO  : send to config file  // this visulizes the frl path as ground truth
    map_theta = MAP_THETA;
    map_theta_optimize = static_cast<bool>(MAP_THETA_OPTIMIZE);  //TODO  : send to config file  // this visulizes the ppk path as ground truth
    std::string ppk_pos_file = PPK_PATH; //TODO  : send to config file
    std::string frl_pos_file = FRL_PATH; //TODO  : send to config file
    myfile1.open(ppk_pos_file);
    myfile2.open(frl_pos_file);

    cout << "PPK file: " << ppk_pos_file<< endl;
    cout << "FRL file: " << frl_pos_file<< endl;
    cout << "viz FRL: " << viz_frl << endl;
    cout << "use FRL: " << use_frl << endl;
    

    globalEstimator.set_map_theta(map_theta/180.0*PI,map_theta_optimize);
    //globalEstimator.map_theta = map_theta/180*PI;  //use this every where not the above
    //globalEstimator.map_theta_optimize = map_theta_optimize;
    cout << "MAP_THETA: " << globalEstimator.map_theta*180.0/PI << endl;
    cout << "MAP_THETA_OPTIMIZE: " << globalEstimator.map_theta_optimize << endl;
